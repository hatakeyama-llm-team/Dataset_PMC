...
def remove_xref_and_wrapped_brackets(text, proximity=10):
    try:
        ...
        for xref in xrefs:
            ...
            full_text = (ET.tostring(parent, method='text', encoding='unicode', with_tail=False) or "")
            ...
            pre_slice = full_text[max(0, xref_start - proximity):xref_start] if xref_start != -1 else ""
            post_slice = full_text[xref_end:xref_end + proximity] if xref_end != -1 else ""

            # Regex to find the closest brackets around xref
            pre_bracket_match = re.search(r'(\(|\[)[^\(\)\[\]]*$', pre_slice) if pre_slice else None
            post_bracket_match = re.search(r'^[^\(\)\[\]]*(\)|\])', post_slice) if post_slice else None
            ...
            if pre_bracket_match and post_bracket_match:
                ...
                if (pre_bracket, post_bracket) in [('(', ')'), ('[', ']')]:
                    ...
                    new_pre_slice = pre_slice[:pre_bracket_index] + pre_slice[pre_bracket_index+1:]
                    new_post_slice = post_slice[:post_bracket_index] + post_slice[post_bracket_index+1:]

                    # Update the parent text to exclude the xref and the brackets
                    parent.text = (parent.text or "")[:max(0, xref_start - proximity)] + new_pre_slice + (parent.text or "")[xref_start:] + new_post_slice
            else:
                ...
                if xref.tail:
                    ...
                    if xref.getprevious() is not None:
                        xref.getprevious().tail = (xref.getprevious().tail or "") + (xref.text or "")
                    else:
        ...
        cleaned_text = "".join(cleaned_text.splitlines())[6:-7].strip()  # Remove artificial root element
        return cleaned_text
...
def extract_abstract_and_body_text(xml_string):    
    try:
        ...
        abstract_text = ' '.join(ET.tostring(element, method='xml', encoding='unicode') for element in abstract_elements)
        body_text = ' '.join(ET.tostring(element, method='xml', encoding='unicode') for element in body_elements)
        ...
        abstract_text = remove_unnecessary_elements(f"<root>{abstract_text}</root>", remove_list)
        body_text = remove_unnecessary_elements(f"<root>{body_text}</root>", remove_list)
